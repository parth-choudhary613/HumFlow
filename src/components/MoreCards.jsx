    ...
    return (
      <div className="flex justify-center p-4">
        <button
          ...
          className="w-86 lg:w-240 md:w-176 border border-white text-white text-sm font-medium rounded-lg backdrop-blur-lg py-2 px-4 shadow-sm bg-transparent"
        >
          More Sounds
        </button>
      </div>
    ...
    <div className="flex flex-col items-center justify-center p-4">
      <div className="mt-6 flex flex-col gap-3 w-86 lg:w-300 md:w-176 ">
        {moreData.map((item) => (
          <div
            key={item.id}
            className="border border-white/40 p-3 rounded-lg w-full mx-auto bg-white/5 backdrop-blur-md flex items-center gap-2"
          >
            <img
              src={item.thumbnail}
              alt={item.title}
            ...
            <div className="flex-1 flex flex-col sm:flex-row sm:items-center w-full">
              <div className="flex items-center gap-10 sm:gap-4 w-full sm:w-auto">
                ...
                <h3 className="text-white font-sm px-2 py-1">
                  {item.title}
                </h3>
              </div>
